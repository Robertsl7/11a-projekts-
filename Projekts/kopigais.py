...
print("Sporta centra abonementu programma")
class klienta_informacija():
    ...
    def abonesana(self):
        ...
        print("Abonementa izveides datums", self.abonesanas_laiks.strftime("%Y-%m-%d"))
        izveleties_abonementu()
        abonesanas_veids = int(input("Jūsu izvēle: "))

        while True:
            if abonesanas_veids == 1:
                self.abonesanas_beigsanas = abonesanas_laiks + timedelta(days=1)# 1 dienas abonements
                print("\nAbonementa beigu datums: ", self.abonesanas_beigsanas.strftime("%Y-%m-%d"))
                break
            if abonesanas_veids == 2:
                self.abonesanas_beigsanas = abonesanas_laiks + timedelta(days=7)# 7 dienu abonements
                print("\nAbonamenta beigu datums: ", self.abonesanas_beigsanas.strftime("%Y-%m-%d"))
                break
            if abonesanas_veids == 3:
                self.abonesanas_beigsanas = abonesanas_laiks + timedelta(days=30)# 30 dienu abonements
                print("\nAbonamenta beigu datums: ", self.abonesanas_beigsanas.strftime("%Y-%m-%d"))
                break
            if abonesanas_veids == 4:
                self.abonesanas_beigsanas = abonesanas_laiks + timedelta(days=365)# 365 dienu abonements
                print("\nAbonamenta beigu datums: ", self.abonesanas_beigsanas.strftime("%Y-%m-%d"))
                break
    ...
    def failu_izveide(self):
        ...
        with open(faila_nosaukums,"w", encoding = "utf-8") as file:
            ...
            file.write(f"\nAbonamenta izveides datums: {self.abonesanas_laiks}\nAbonementa beigšanās datums: {self.abonesanas_beigsanas}")
        

def izveleties_abonementu():# Izvada abonementa izvēles iespējas
    print("\nIzvēlaties abonamenta veidu: \n1- dienas abonements \n2- nedēļas abonements \n3- mēneša abonements \n4- gada abonements")
    
def atcelt_abonamentu():# Ļauj dzēst abonementu, ievadot faila nosaukumu
    filename = input("Ievadiet faila nosaukumu, no kura vēlaties dzēst abonementu(vards_uzvards.txt): ")
    ...
    print("Abonements ir dzēsts.") 

def galvena():#Galvenā funkcija pie atbilstošas izvēles izsauc atbilstošu funkciju.
    while True:
        print("Ko jūs vēlaties darīt? (1- izveidot jaunu profilu, 2- dzēst abonementu, 3- iziet):")
        izvele = input()
        if izvele == '1':
